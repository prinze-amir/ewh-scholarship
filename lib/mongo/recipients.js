...
async function connectToDatabase() {
    ...
    async function getLatestRecipient() {
        const collection = await connectToDatabase();
        const [latestApproved] = await collection
        ...
        // Fallback for installs that do not have approved recipients yet.
        const [latestAny] = await collection
            .find({})
            .sort({ createdAt: -1, _id: -1 })
            .limit(1)
            .toArray();

        return latestAny || null;
    }
     async function getRecipient(id) {
        const objectId = new ObjectId(id);
        const collection = await connectToDatabase();
        const recipient = await collection.findOne({_id: objectId});
        return JSON.parse(JSON.stringify(recipient));
    }
